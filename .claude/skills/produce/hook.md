...
| Background activity 🤖 | 4×4 grid cells show mean-abs-luminance-delta (frame0 vs mid-hook frame) >5; must satisfy one of: (A) ≥2 active cells spatially separated (Chebyshev distance ≥2); OR (B) concentrated focal: highest active-cell delta >10 (≈2× floor). Frozen/low-motion single region (max delta 5–10) fails both paths. **Use one of the ambient-field motifs from `src/lib/fx.tsx` as the living-background layer (`AmbientField`/`MoteField`/`GridPulse`/`EmberRise` — all pass by construction); see `hooks.md` motif palette and `AmbientCheck` fixture for reference PASSes.** |
