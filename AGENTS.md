...
- `node scripts/new-video.mjs <slug> <CompId> [--hook=<key>] [--body=<key>] [--ambient=<key>] [--distinct]` — scaffold new video skeleton (hook-gate-green by construction: P1/P2 + living-background motif + valid starter palette + Hook scene with promise.text; --hook=<key> instantiates a gate-PASS archetype Hook.tsx from the 8-archetype catalog; --body=<key> instantiates retention-gate-green Body/Climax|Cta scenes from the 9-pattern catalog; --ambient=<key> wires a living-background motif (strips/motes/grid-pulse/ember-rise) into all emitted scenes; --distinct emits anti-convergence seed palette+fonts+motif from the registry — distinct-gate-green by construction — run with unknown key to list valid keys)
