...
- `node scripts/new-video.mjs <slug> <CompId> [--hook=<key>]` — scaffold new video skeleton (hook-gate-green by construction: P1/P2 + AmbientField living-background + valid starter palette + Hook scene with promise.text; --hook=<key> instantiates a gate-PASS archetype Hook.tsx from the 8-archetype catalog — run with unknown key to list valid keys)
