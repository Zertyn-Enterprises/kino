...
| `node scripts/new-video.mjs <slug> <CompId> [--hook=<key>] [--body=<key>] [--distinct]` | scaffold a new video skeleton (P1/P2-passing by construction); `--hook=<key>` instantiates a gate-PASS archetype Hook.tsx; `--body=<key>` instantiates retention-gate-green Body/Climax\|Cta scenes; `--distinct` emits anti-convergence seed palette+fonts — distinct-gate-green by construction |
