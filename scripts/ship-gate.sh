#!/usr/bin/env bash
# Runs the full ship gate: hook.sh + retention.sh + contrast.sh + motion.sh + legibility.sh + code-craft.sh + musicsync.sh + payoff.sh + remotion-correct.sh + distinct.sh + registry-sync.sh, aggregates results.
...
REGISTRY_SYNC_JSON="out/review/registry-sync/metrics.json"
...
scripts/registry-sync.sh "$SLUG" >"$GATE_TMPDIR/registry-sync.log" 2>&1 &
REGISTRY_SYNC_PID=$!
...
wait "$REGISTRY_SYNC_PID"       || REGISTRY_SYNC_EXIT=$?
...
echo ""
echo "==> registry-sync gate:"
cat "$GATE_TMPDIR/registry-sync.log"
echo ""

# --- 2. Aggregate via ship-metrics.mjs ---

SHIP_EXIT=0
node scripts/ship-metrics.mjs "$HOOK_JSON" "$RETENTION_JSON" "$CONTRAST_JSON" "$MOTION_JSON" "$LEGIBILITY_JSON" "$CODE_CRAFT_JSON" "$MUSICSYNC_JSON" "$PAYOFF_JSON" "$REMOTION_CORRECT_JSON" "$DISTINCT_JSON" "$REGISTRY_SYNC_JSON" --json \
  > "$SHIP_OUT/report.json" || SHIP_EXIT=$?
node scripts/ship-metrics.mjs "$HOOK_JSON" "$RETENTION_JSON" "$CONTRAST_JSON" "$MOTION_JSON" "$LEGIBILITY_JSON" "$CODE_CRAFT_JSON" "$MUSICSYNC_JSON" "$PAYOFF_JSON" "$REMOTION_CORRECT_JSON" "$DISTINCT_JSON" "$REGISTRY_SYNC_JSON" \
